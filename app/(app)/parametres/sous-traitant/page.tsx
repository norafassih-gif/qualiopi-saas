...
export default async function SousTraitantPage({
  ...
  const partner = await getMyFirstPartner("sous_traitant");
  const { saved } = await searchParams;

  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <h1 className="mb-1 text-2xl font-bold">Mon sous-traitant</h1>
      <p className="mb-6 text-sm text-gray-600">
        Ces informations complètent automatiquement le Contrat de
        sous-traitance d&apos;enseignement (indicateur 27) — plus besoin de
        les ressaisir à la main à chaque génération.
      </p>
      {saved === "1" && (
      ...
      <PartnerForm partnerType="sous_traitant" partner={partner} />
