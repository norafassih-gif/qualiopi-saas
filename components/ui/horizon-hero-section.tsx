...
// Hero plein écran animé (étoiles, nébuleuse, montagnes en parallaxe, effet de
// lumière) — inspiré d'un composant de la galerie 21st.dev, converti en
// TypeScript et adapté à Next.js (App Router). Purement visuel : aucune
// donnée, aucun appel réseau, aucune IA — aligné avec la contrainte du projet
// de ne dépendre d'aucun service payant en production.
...
type SubtitleLines = { line1: string; line2: string };

export const HorizonHeroSection: React.FC<{ children?: React.ReactNode }> = ({
  children,
}) => {
  ...
  // Animations GSAP — jouées une fois la scène 3D prête, pour éviter tout flash
  useEffect(() => {
    if (!isReady) return;
  ...
  // Défilement — fait avancer la caméra et le parallaxe des montagnes
  useEffect(() => {
    const handleScroll = () => {
      ...
      const refs = threeRefs.current;
      if (!refs.nebula || refs.mountains.length < 4) return;
    ...
    >
      ...
      <div className="scroll-sections">
        {[...Array(2)].map((_, i) => {
          ...
          const subtitle = subtitles[sectionIndex];
          return (
            <section key={i} className="content-section">
              <h1 className="hero-title">{titles[sectionIndex] || "DEFAULT"}</h1>
